...
//======================================================//
if (localStorage["Player_Jump"] != undefined)
{
    playerNumber = parseInt(localStorage["Player_Jump"]);
}

if (localStorage["Theme_Jump"] != undefined)
{
    themeNumber = parseInt(localStorage["Theme_Jump"]);
}

if (localStorage["Sounds_Jump"] != undefined)
{
    sounds = parseInt(localStorage["Sounds_Jump"]); //0 for off and 1 for on
}
   ...
   startButton.onclick = function ()
   {
        ...
        function Preloader(game)
    {
        ...
        this.percentageText = percentageText;
            ...
            {
              // Images for plateform, coins and fireballs (obstacles)
              var imagesList = [
              ...
              { name: "platform-3", path: "images/platform-3.png" },
...
girl.MovableGameObject = (function () {
    ...
    return MovableGameObject;
        ...
        {
            if (level == 1) {
                var image = new createjs.Bitmap("images/platform-left.png");
            }
            else if (level == 2) {
                var image = new createjs.Bitmap("images/platform-right.png");
            }
            else if (level == 3) {
                var image = new createjs.Bitmap("images/platform-right.png");
            }
        ...
        else if (width > 120)
        {
            if (level == 1) {
                ...
                var imageLeft = new createjs.Bitmap("images/platform-3.png");
                this.addChild(imageLeft); // width 57
                var imageRight = new createjs.Bitmap("images/platform-3.png");
                this.addChild(imageRight); // width 62
                var imageMiddle = new createjs.Bitmap("images/platform-middle.png");
                this.addChild(imageMiddle);
            }
...
// let's make game little bit interesting
        p.initGame = function () {
        this.resetGame();
        ...
        var lastPlatformX = 5;
        var lastPlatformY = 50;

        //level 1 Easy one so many coins
        for (var i = 0; i < 15; i++)
        {
            var width = 120 + Math.round(Math.random() * 5);
            ...
            lastPlatformY = platform.y;
                ...
                if (Math.random() > 0.9 && i >= 1) {//Pav Singh
        ...
        for (var i = 0; i < 25; i++) {
            ...
            var gapBetweenPlatforms = Math.random() * 37;
            lastPlatformX += platform.width + gapBetweenPlatforms;
            lastPlatformY = platform.y;

            // let's put an obstacle on platform.
            if (Math.random() > 0.5 && i >= 1) {//Pav Singh
                var obstacle = new girl.Obstacle();
                obstacle.x = platform.x + platform.width / 2;
                obstacle.y = platform.y;
                this.camera.addChild(obstacle);
            ...
            this.camera.addChild(platform);
            var gapBetweenPlatforms = Math.random() * 40;
            lastPlatformX += platform.width + gapBetweenPlatforms;
            lastPlatformY = platform.y;

            // let's put an obstacle on platform.
            if (Math.random() > 0.1 && i >= 1) {//Pav Singh
                var obstacle = new girl.Obstacle();
                obstacle.x = platform.x + platform.width / 2;
                obstacle.y = platform.y;
                this.camera.addChild(obstacle);
    ...
    p.updateView = function ()
    {
        ...
        this.scoreHud.innerHTML = "<img src='images/coin1.png' />&nbsp;" + this.collectedCoins + "* 100 &nbsp;&nbsp;&nbsp;&nbsp;<img src='images/fireIcon.png' width='25' height='25'&nbsp; /> " + this.jumpedObstacle  + "* 50 &nbsp;"
            ...
            + "Level : " + level +" &nbsp;"
            + "Energy : " + energy+" %"
            + "<span style='float:right;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Player : " + playerName + "</span><span style='float:right;'> &nbsp;&nbsp;&nbsp;Score :" + this.collectedCoins + "</span>";
    ...
    p.gameOver = function () {
        ...
        var maxScore = Math.max(localStorage['maxScore'], this.collectedCoins * 100 + this.jumpedObstacle*5);
        localStorage['maxScore'] = maxScore;

        yourScore.innerHTML = '<br/><br/>Your score is ' + (this.collectedCoins * 100 + this.jumpedObstacle*5) + '.';

        if ((this.collectedCoins * 100 + this.jumpedObstacle * 5) >= maxScore)
        {
            yourScore.innerHTML += '<br/><br/><b>Congrats!!! You made highest Score. </b>';
            tweet.href = 'http://twitter.com/share?text=I scored ' + (this.collectedCoins * 100 + this.jumpedObstacle * 5) + ' points which is the highest in the 21 jump street Game by sharn...';
            fbText = 'I scored' + (this.collectedCoins * 100 + this.jumpedObstacle * 10) + ' points which is the highest in 21 jump street by Sharn...';
        }
        else
        {
            tweet.href = 'http://twitter.com/share?text=I scored ' + (this.collectedCoins * 100 + this.jumpedObstacle * 5) + ' points in the 21 jump street Game by sharn...';
            fbText = 'I scored' + (this.collectedCoins * 100 + this.jumpedObstacle * 5) + ' points in the 21 jump street Game by Sharn...';
        ...
        topScores.saveScore((this.collectedCoins * 100 + this.jumpedObstacle * 5), playerName);
